...
//            IMLog.i("tencent_im_sdk_plugin_res",map.toString());
    ...
    public static void downloadFileMessage(final String msgID, V2TIMFileElem elem, final DownloadCallback callback){
        ...
        try {
            ...
            if (cacheDir.exists()) {
                final String path = cacheDir.getPath() + "/file_"+ elem.getUUID()+(elem.getUUID().contains(".")?"":"_temp_"+elem.getFileName());
                final String finalPath = cacheDir.getPath() + "/file_"+ elem.getUUID()+(elem.getUUID().contains(".")?"":"_"+elem.getFileName());
                final File tempFile = new File(path);
                final File finalFile = new File(finalPath);
                if(!finalFile.exists()){
    ...
    public static HashMap<String,Object> convertV2TIMGroupChangeInfoToMap(V2TIMGroupChangeInfo info){
        ...
        rinfo.put("intValye",info.getIntValue());
        return rinfo;
    }
    ...
    public static List<String> checkApiContains(String t){
        List<String> list = new LinkedList<>(Arrays.asList("TencentCloudAVChatRoom","TencentCloudChatVote"));
